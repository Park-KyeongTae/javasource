...
public class OrderDAO {
	...
	public boolean insert(int userId, int productId) {
		...
		try {
			...
			con = getConnection();
			String sql = "INSERT INTO SORDER(order_id,user_id,product_id,order_date) VALUES(SORDER_SEQ.NEXTVAL,?, ?, SYSDATE)";
			// ?
	...
	public List<OrderDTO> carts(int userId) {
		...
		try {
			...
			String sql = "SELECT U.USER_ID, U.NAME, T.INFO, S.PRODUCT_ID, P.PNAME, P.PRICE, P.CONTENT, S.ORDER_DATE "
					+ "FROM SORDER S, SUSER U , paytype T , PRODUCT P "
					+ "WHERE U.USER_ID = U.USER_ID AND U.PAY_NO = T.PAY_NO AND S.PRODUCT_ID = P.PRODUCT_ID AND S.USER_ID = ?";

			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, userId);
			rs = pstmt.executeQuery();

			// ResultSet 결과 : user_id, name, info, product_id, pname, price, content,
			// order_date ==> OrderDTO 담기
